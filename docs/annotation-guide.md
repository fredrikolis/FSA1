...
It lives in the repo because Gate B grades against it, and a reviewer in a fresh context has no
`annotated-tree` to ask. A rubric it cannot open is a rubric it invents.
...
Example:  // Concern: runs the core loop | Non-concern: transport | IO: (Job) -> Result
