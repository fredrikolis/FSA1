...
- **CLI1 · self-contained tool.** The tool is charlie-cli, a command-line program; everything needed to author, render, validate, and repair a workbook is reachable from charlie-cli alone. — *`charlie-cli --guide` states the format's rules, `charlie-cli sample` writes a workbook that loads and renders, `charlie-cli check` validates it, and `charlie-cli set` writes a cell or range into it — a validated authoring path that produces the same on-disk grid, so the filesystem stays the source of truth; every refusal carries its location (CORE2); an agent given only charlie-cli — no companion document, repository, or service — can author a valid workbook and repair a rejected one.*
