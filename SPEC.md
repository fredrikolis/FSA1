...
- **ENG3 · two-pass, contained evaluation.** Evaluation is two-pass: a *plan* pass builds one dependency graph of the cells a render demands — built up and merged as render ops are added, so a shared dependency appears once — and an *evaluate* pass computes each cell in dependency order (compute-once is ENG2). The graph is a contained optimization: its type is private to the engine, and it equals a naive per-cell evaluation. — *a differential test evaluates complex dependency chains both ways — naive per-cell and two-pass — and asserts identical values; the dependency-graph type appears in no other module's surface; removing the sharing changes performance, never results. When a cell's references are resolved by a source rewrite before planning — a name (FS4), or a reference-forging call (INDIRECT/OFFSET, ENG6) resolved to static references — the passes operate on that rewritten effective form and the naive oracle applies the same rewrite, so two-pass still equals naive; the dependency graph reflects the resolved (effective) references.*
...
- **CLI3 · tree (workbook structure).** charlie-cli renders the workbook as an annotated tree — its tabs, its cell/range files, and its names — each entry shown with what it holds. — *`charlie-cli tree` walks the workbook's on-disk layout (tabs are folders, cell/range files are named by their A1 range, FS1–FS2) and prints one node per entry, annotated by that entry's kind: a **tab** (folder) holds its child entries, shown as its subtree; a **cell/range file** carries its authored cells — the formula(s) or literal(s) a deserializer reads and an agent edits (GRID2), not evaluated values (that is `render`); a **name** (FS4) carries what it names, resolved at load — a cell or range reference, or (for a named formula or constant) its authored definition — since a name is not a grid file (GRID1) and, like a cell, is shown unevaluated. The reserved `.cache/` directory (FS3) is derived, non-authoritative data and is never shown. It is a read-only view (CORE3 — `tree` never mutates the workbook). The tree's edges are exactly the on-disk structure (a folder's entries and sub-folders), not the engine's dependency relation — `tree` shows what exists and where, `trace` (CLI2) shows what flows. Cell/range files within a tab are listed in A1 order (by row, then column) so the view is stable and scannable; multi-row content is presented so each entry stays a single node. `tree` emits a single view — the one rendering an agent should read — not a choice of output formats: a second machine-readable form would only invite a consumer to re-derive its own tree from it, so the optimized rendering is the sole output. How that view is drawn is a presentation detail, not a spec fact.*
