...
FILE BODY — a TSV grid, one cell per coordinate
  The file is the grid: no header/annotation line, line 1 is the first row. A trailing
  @scope { … } block holds no cell and shifts no grid line. check parses and validates one
  and reports its faults; render --format html carries it into the document's CSS, and pack
  carries it into the .xlsx as fonts, fills, borders, alignment, column widths and row heights.
