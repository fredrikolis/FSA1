// Concern: spells the formula bar the document splices in | Non-concern: which cell is focused, what a cell's address or formula IS | IO: none -> markup, CSS and a script
...
fsa1-cell:focus { outline: 2px solid #3f6fb7 }
";

/// Reads `data-ref` and `data-formula` off the focused cell and shows its own text where it holds no
/// formula. One listener on the document, so a sheet of any size costs one handler.
pub(crate) const SCRIPT: &str = "\
document.addEventListener('focusin',function(e){\
var c=e.target.closest&&e.target.closest('fsa1-cell');if(!c)return;\
