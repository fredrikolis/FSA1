// Concern: interns each distinct CellStyle under one class and spells the <style> block | Non-concern: which cell holds a style, the table markup | IO: (CellStyle) -> a class; (the interned set) -> CSS
...
/// `border-collapse` is load-bearing: it hands the browser CSS 2.1 §17.6.2.1 conflict resolution over
/// the edge two adjacent cells share, so each cell declares only its OWN borders and no shared edge is
/// pre-resolved here. `pre-wrap` makes a cell's embedded newlines and spaces read as they do in the
/// ASCII carrier. Both are element-selector defaults a cell's own class outranks.
const BASE: &str = "\
...
impl Classes {
    ...
    pub(crate) fn intern(&mut self, style: CellStyle) -> Option<String> {
