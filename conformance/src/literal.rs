// Concern: the FIXTURE LITERAL grammar — the single, shared textual form for both a context cell's value and a fixture's EXPECTED value: parse a literal token (`6`, `2.5`, `"text"`, `TRUE`/`FALSE`, an `#…!` error, `<blank>`/empty, a `{r,c;r,c}` array) into a charlie-ast `Value`, and render a `Value` back for a human diff detail | Non-concern: the fixture RECORD grammar (corpus.rs owns `[name]`/`funcs:`/`formula:`/`cell …`) and evaluating a formula (grade.rs) — this only converts one literal ↔ one `Value` | IO: (&str) -> Result<Value, String>, and (&Value) -> String
//! The literal grammar shared by a fixture's context cells and its EXPECTED value. Deliberately
//! narrow and Excel-flavored: it is authored by a human deriving a value from the spec, so it must be
//! unambiguous and total (a malformed literal is a fail-fast corpus error, never a silent default).

use charlie_ast::{ErrKind, Shape, Value};

/// Parse one literal into a [`Value`]. Fail-fast (`Err`) on anything not in the grammar — a corpus
/// author's typo must surface loudly, never coerce to a wrong value.
...
/// A `"…"` string with Excel `""`-escaping. `rest` is everything after the opening quote.
fn parse_text(rest: &str) -> Result<Value, String> {
    ...
    while i < bytes.len() {
        if bytes[i] == b'"' {
            // `""` is an escaped quote; a lone `"` must be the terminator (nothing after it).
        ...
        // Copy one UTF-8 char (indices track bytes; push the char at this boundary).
        let ch = rest[i..]
            .chars()
            .next()
...
/// One of the nine spreadsheet error literals.
...
/// A `{r1c1,r1c2;r2c1,r2c2}` array — rows separated by `;`, columns by `,`, each element a scalar
/// literal (no nested arrays). Row-major, ragged rows rejected.
...
/// Render a [`Value`] back to its literal form for a human diff detail. Inverse-ish of [`parse`]
/// (a number uses Rust's shortest round-tripping form, which may differ cosmetically from the
/// authored literal — this is a DIAGNOSTIC render, not a canonicalizer).
...
/// The canonical text of an error kind (mirrors [`parse_error`]).
...
mod tests {
    ...
    fn text_handles_escaping_and_spaces() {
        ...
        // A numeric-looking string stays text when quoted.
