# Concern: the ENG6 WHOLE-WORKBOOK differential harness (DoD milestone 3) — for each real .xlsx under corpus_workbooks/, (a) compute it with the `formulas` reference lib and collect {sheet!cell -> value} for EVERY formula cell, (b) `charlie-cli import` it into a temp charlie workbook then `charlie-cli eval <wb> --tab <sheet> --formula =<cell>` for each such cell, and (c) diff cell-for-cell reusing oracle.py's type-driven comparison + per-value tolerance; print a per-workbook parity table + an overall summary, and exit non-zero on any charlie-defect divergence | Non-concern: WHAT charlie computes or WHY (charlie-model/charlie-ast own that), the per-formula corpus grading (oracle.py owns corpus/*.json), authoring the workbooks (make_workbooks.py), and the reference stack pin (requirements.txt); a genuine `formulas` lib defect is triaged into KNOWN-LIB-GAPS.md and excluded, never chased by editing charlie | IO: (in: corpus_workbooks/*.xlsx, the charlie-cli binary, a temp workdir) -> a parity table + match/total summary on stdout, exit 0 iff no charlie-defect divergence
"""ENG6 whole-workbook oracle: grade charlie against `formulas` on real multi-formula .xlsx files.

Run via ``run.sh --workbook`` (which activates the venv and builds charlie-cli). This is the real
'import -> compute -> diff vs reference' fitness on genuine files, complementing the per-formula
``oracle.py``. It reuses oracle.py's ``compare``/``classify_ref``/``charlie_value`` so the value
...
import oracle  # sibling module: reuse its comparison + charlie eval (single source of truth)
...
def load_lib_gaps():
    ...
    each is evidenced in KNOWN-LIB-GAPS.md. Distorting charlie to a wrong reference is forbidden."""
...
def formula_cells(xlsx_path):
    ...
    charlie tab folder name), coord like ``A3``. A formula cell is one openpyxl marks data_type 'f'."""
...
def grade_workbook(cli, xlsx_path, workdir, lib_gaps):
    ...
    ``(sheet, cell, verdict, ref_disp, charlie, detail)`` for the parity table.
    """
    wb_dir = workdir / "cwb"
    proc = subprocess.run(
    ...
    if proc.returncode != 0:
        ...
        # An import-level failure is a charlie defect (it forces a non-zero exit) but it is NOT a graded
    ...
    for sheet, coord in formula_cells(xlsx_path):
        ...
        ch_value, _exit, _note = oracle.charlie_value(cli, wb_dir, f"={coord}", tab=sheet)
...
def _print_table(title, rows):
    print(f"\n=== {title} ===")
    headers = ("sheet", "cell", "verdict", "reference", "charlie", "detail")
...
def run():
    cli = Path(os.environ.get("CHARLIE_CLI", oracle.DEFAULT_CLI))
    if not cli.exists():
    ...
    if total_defects:
        print(f"DIVERGENCES (charlie defects): {total_defects}")
    else:
