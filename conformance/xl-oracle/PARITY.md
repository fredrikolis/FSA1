...
**Per-formula result: 542 / 542 graded cases MATCH (parity = 100.0%). 0 charlie-defect divergences. 33 cases EXCLUDED (lib-gaps).**
...
charlie defect — charlie matches real Excel on each, hand-verified per the ENG6 triage rule (the corpus
grades charlie against Excel; where the reference is wrong, charlie is not distorted to match it). The
...
Not counted as gaps: `INDIRECT`/`OFFSET` are deliberate charlie parse-refusals (reserved for a later
