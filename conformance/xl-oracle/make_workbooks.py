# Concern: authors the committed .xlsx corpus for the whole-workbook oracle | Non-concern: computing or diffing values, the per-formula corpus | IO: () -> corpus_workbooks/*.xlsx
...
so both the `formulas` reference and FSA1 (after `fsa1-cli unpack`) evaluate the same source.
"""
...
def make_forging():
    ...
    addressing' workhorses. FSA1 source-rewrites each forger to a static reference before evaluation;
    the `formulas` reference computes them natively, so both sides agree on the forged value. NESTED
    forging (a forger whose argument forges) is a deliberate FSA1 divergence (a located #REF!,
    restricted v1) and so is deliberately NOT authored here (the parity corpus grades supported cases;
    the refusal is pinned by the fsa1-model forge fitness tests)."""
    wb = openpyxl.Workbook()
    s = wb.active
    s.title = "Forge"
