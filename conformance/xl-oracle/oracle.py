# Concern: the per-formula differential harness — the formulas lib vs FSA1 per case | Non-concern: what FSA1 computes, whether the lib is right | IO: (corpus/*.json) -> a parity table
"""ENG6 differential oracle: grade FSA1 cell-for-cell against the `formulas` Excel reference.
...
FSA1 value comes from ``fsa1-cli eval``. Comparison is driven by the *reference's* value type
...
DEFAULT_CLI = REPO_ROOT / "target" / "debug" / "fsa1-cli"
TAB = "S"  # single tab, in both the xlsx sheet and the FSA1 tab folder
...
def build_fsa1_workbook(inputs, workdir):
...
def _grid_literal(literal):
    """Render a corpus literal as FSA1 grid content (a bare number/text cell — no annotation)."""
    if isinstance(literal, bool):
        return "TRUE" if literal else "FALSE"
    return str(literal)


def fsa1_value(cli, wb_dir, formula, tab=TAB):
    """Run ``fsa1-cli eval`` and return ``(value_str_or_None, exit_code, note)``.
    ...
    """
    # The tab is addressed BY PATH (`<wb>/<tab>`) — plan 08 removed the `--tab` flag in favour of
    # fs-native addressing. Passing the old flag makes every case exit 2 with an empty stdout, which
    # this harness would otherwise report as 539 FSA1 "defects".
    proc = subprocess.run(
        [str(cli), "eval", str(Path(wb_dir) / tab), "--formula", formula],
        capture_output=True,
        text=True,
    )
    out = proc.stdout.strip()
    # A USAGE failure (exit 2 — bad args, unknown flag/verb) is a BROKEN HARNESS, not an FSA1 answer.
    # Left ungraded it silently becomes "FSA1 value '' is not numeric" on every case: that is exactly
    # how a removed `--tab` flag turned into 539 phantom "FSA1 defects" that nobody read as a harness
    # break. Abort loudly instead — a construct-not-run must never share a verdict with a wrong answer.
    if proc.returncode == 2 and not out:
        raise RuntimeError(
            "fsa1-cli rejected the invocation (exit 2) — the harness, not FSA1, is broken: "
            + (proc.stderr.strip()[:200] or "<no stderr>")
        )
    # Text is FSA1's sole output form: `eval` prints the scalar value (a plain value OR an
...
def compare(case, ref_value, fsa1_str):
    """Compare FSA1's raw output string against the reference, driven by the reference's kind.

    Returns ``(is_match, ref_display, kind, detail)``.
    """
    kind, canonical = classify_ref(ref_value)
    if kind == "error":
        ok = fsa1_str == canonical
        return ok, canonical, kind, "" if ok else "FSA1 did not reproduce the error value"
    if kind == "bool":
        ok = fsa1_str == canonical
        return ok, canonical, kind, "" if ok else "boolean mismatch"
    if kind == "num":
        if fsa1_str is None:
            return False, _fmt_num(canonical), kind, "FSA1 produced no value (refusal)"
        try:
            got = float(fsa1_str)
        except ValueError:
            return False, _fmt_num(canonical), kind, f"FSA1 value {fsa1_str!r} is not numeric"
...
def run():
    cli = Path(os.environ.get("FSA1_CLI", DEFAULT_CLI))
    if not cli.exists():
    ...
    try:
        for case in cases:
            ...
            wb_dir = build_fsa1_workbook(case["inputs"], workdir)
            ch_value, ch_exit, ch_note = fsa1_value(cli, wb_dir, case["formula"])
    ...
    if defects:
        print(f"DIVERGENCES (FSA1 defects): {defects}")
    else:
...
def _print_table(rows):
    headers = ("category", "case", "verdict", "reference", "FSA1", "detail")
