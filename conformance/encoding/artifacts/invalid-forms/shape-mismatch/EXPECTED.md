...
GRID4 requires the deserialized grid to fill the declared closed range exactly. The body is `2×3` but
the range declares `3×3`, so the grid does not fill it. There is no broadcast or fill mechanism: a
range file is an explicit grid (VAL1), so a `2×3` body into a `3×3` range is simply short. Charlie
