<!-- Concern: how the invoice-aging ground truth was computed, independently of FSA1 | Non-concern: the FSA1 encoding under test (the workbook itself lives in ../../artifacts/dates/invoice-aging/) | IO: none -->
# PROVENANCE — oracle/dates/invoice-aging

Ground truth for `artifacts/dates/invoice-aging/` (tab `Invoices`), rendered values in
`invoice-aging.oracle.csv`. Computed **independently of FSA1** (ORACLE-INPUT PURITY: FSA1
cannot evaluate yet and must never grade itself).

## Method

Plain Python 3 with the stdlib `datetime` module only — no spreadsheet engine, no FSA1. The exact
...
- **Date system**: 1900 serial date system (the spreadsheet default), computed with epoch
  ...
  dates and what FSA1's `DATE()` is expected to return. `2026-03-31` -> serial `46112`.
  Note: the day-count (`DATEDIF(...,"d")`) is a *difference* of serials, so it is independent of the
  epoch choice — only the raw serial literals in column C depend on it. If FSA1 adopts a different
