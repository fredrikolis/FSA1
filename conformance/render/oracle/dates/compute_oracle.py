# Concern: independent ground-truth day-counts and aging-bucket rollups for the dates/invoice-aging workbook, cross-checked two ways (if-ladder vs COUNTIFS/SUMIFS criteria) | Non-concern: FSA1's DATE/DATEDIF/IFS implementation (that engine lives in the FSA1/ submodule) — computed here with the stdlib datetime so the engine is never its own grader | IO: (in: pinned invoice rows + reference date restated in-file) -> prints per-row serials/day-counts/buckets and reconciling rollups to stdout
