#!/usr/bin/env python3
# Concern: independent ground-truth rendered cell values for the aggregation/sales_report workbook (input echo, SUM/SUMIFS/COUNTIFS/AVERAGEIFS rollups) | Non-concern: charlie's own formula evaluation (that engine lives in the charlie/ submodule) — recomputed here in pandas so charlie is never graded against its own output | IO: (in: input-cell records restated in-file, mirroring the .range inputs) -> writes expected_values.json + expected_derived.csv beside this script
"""Independent oracle for the aggregation/sales_report charlie workbook.

Provenance-pure: ground truth is computed HERE from the input data with pandas/
plain arithmetic. Charlie never touches this — charlie cannot evaluate yet, and
...
# ---- INPUT (mirrors the .range input files; NOT read from charlie) ----
rows = [
