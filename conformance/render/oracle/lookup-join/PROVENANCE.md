<!-- Concern: how the lookup-join ground truth was computed, for audit/reproducibility | Non-concern: the FSA1 encoding of the sheet (see ../../artifacts/lookup-join/) | IO: none -->
# PROVENANCE — oracle/lookup-join

Ground truth for `artifacts/lookup-join/`, computed **independently of FSA1**
(ORACLE-INPUT PURITY — the tool never grades itself; FSA1 cannot evaluate yet anyway).

## Method

Pure Python (`compute_oracle.py`, stdlib only — no FSA1, no spreadsheet app).
...
fsa1-cli render to grade the engine cell-for-cell (QA-ladder Tier 6, cell-exact).
