# Concern: independent ground-truth rendered values for the loan_amortization workbook (PMT payment, amortization schedule, and summary rollups) via the closed-form Excel PMT identity | Non-concern: FSA1's own PMT/SUM/COUNTIF evaluation (that engine lives in the FSA1/ submodule) — this must NEVER call FSA1, preserving ORACLE-INPUT PURITY | IO: (in: pinned principal/rate/term constants restated in-file) -> writes oracle_values.json + oracle_values.csv beside this script
#
...
# engine, no FSA1. Run:  python3 compute_oracle.py  -> writes oracle_values.json + oracle_values.csv
#
