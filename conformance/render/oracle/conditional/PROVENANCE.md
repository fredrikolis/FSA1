<!-- Concern: how the customer_tiers ground truth was computed, so it is auditable and reproducible | Non-concern: the on-disk FORMAT.md grammar (see the artifact) and charlie's own evaluator (never used here) | IO: none -->
# PROVENANCE — oracle for `artifacts/conditional/customer_tiers`

**Oracle-input purity.** These expected values were computed **independently of charlie** by a
plain-Python reimplementation of the sheet's logic — charlie cannot evaluate yet, and grading the
tool against itself is forbidden. No spreadsheet engine, no charlie CLI, was involved.
...
Run to reproduce (Python 3.12; stdlib only, no pandas/numpy needed) — from the experiment root
(`exploration/experiments/01-encoding-corpus/`):
```
...
F-column value is the evaluated result the charlie engine must reproduce.
