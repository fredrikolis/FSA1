// Concern: the value->text FORMATTING functions (TEXT FIXED DOLLAR) and the Excel NUMBER-FORMAT-CODE engine they render through — section splitting (`positive;negative;zero;text`), the number path (digit placeholders `0`/`#`/`?`, thousands grouping, `,`/`%` scaling, scientific `E±`, fractions `?/?`, quoted/escaped literals + currency), the date/time path (y/m/d/h/s masks with month<->minute disambiguation, 12-hour `AM/PM`, elapsed `[h]/[m]/[s]`), and the 1900 serial<->civil map (with Excel's leap-year bug) TEXT's date render needs; the supported subset is single-sourced in `format_unsupported`, consulted by BOTH the parse-time gate (`validate_text_format`) and eval (`text_fn`) so what parses and what renders never drift | Non-concern: the string built-ins + VALUE/CHAR/CODE parsing (func/text.rs owns those), the registry table + dispatch (func/mod.rs), the ymd->serial inverse `serial_from_ymd` (func/date.rs owns it), and the shared `one_num`/`opt_num`/`opt_bool`/`arg_text` helpers (func/helpers.rs) | IO: (`EvalCtx`, the call's unevaluated arg `Expr`s) -> `Value`; and (a `Value`, a format string) -> the rendered text or a located error value (CORE2)
use super::*;

// ============================ TEXT / FIXED / DOLLAR entry points ============================

/// `TEXT(value, format)` — render `value` through an Excel format code. An error `value` propagates;
/// a `General` format (case-insensitive) is the value's general text; a text `value` coerces to a
/// number when it looks numeric (`TEXT("123","0.00")="123.00"`), else uses the format's text section
/// if present, else passes through UNCHANGED (`TEXT("abc","0.00")="abc"` — a numeric format applies
/// only to numbers, so non-coercible text is returned verbatim). An UNSUPPORTED format (outside the subset `format_unsupported`
/// draws — colours/conditions in `[…]`, or 5+ sections) is `#VALUE!`; a numeric/date section that the
/// value cannot satisfy (a date serial out of the `[1, MAX_SERIAL]` band) is a located `#VALUE!`,
/// never a panic (CORE2).
...
/// factored so the parse-time gate and eval agree on the supported subset.
fn render_text_format(value: &Value, fmt: &str) -> Value {
    ...
    // A text value renders through the format's TEXT section when it has one (the 4th section of a
    // four-section code, or any section carrying an `@` placeholder — Excel treats an `@`-bearing
    // section as the text section wherever it sits, so a lone `"@"` or `"\"n/a\""` formats text). With
    // NO text section, numeric-looking text coerces to a number and renders through the numeric path
    // (`TEXT("123","0.00")="123.00"`); NON-coercible text passes through UNCHANGED — a numeric format
    // only applies to numbers, so already-text that cannot coerce is returned verbatim
    // (`TEXT("abc","0.00")="abc"`, matching Excel and the formulas reference), never `#VALUE!`.
...
/// `FIXED(number, [decimals], [no_commas])` — `number` rounded to `decimals` (default 2) places as
/// text, with `,` thousands grouping unless `no_commas` is TRUE. A NEGATIVE `decimals` rounds to the
/// left of the decimal point (`FIXED(1234.5,-2)="1,200"`). Half-away-from-zero rounding (Excel).
...
/// `DOLLAR(number, [decimals])` — `number` as en-US currency text: a leading `$`, `,` grouping, and
/// `decimals` (default 2) places; a NEGATIVE value is wrapped in parentheses (`DOLLAR(-5)="($5.00)"`),
/// never a leading minus. A negative `decimals` rounds left of the point, as in `FIXED`.
...
/// Pre-round `n` for a (possibly negative) `decimals` count, returning `(value, places)` for
/// [`split_scaled`]: a non-negative `decimals` defers rounding to `split_scaled` (`places = decimals`);
/// a negative `decimals` rounds `n` to the nearest `10^|decimals|` here (half-away) and renders 0
/// places. Shared by `FIXED` and `DOLLAR`. The left-shift exponent is clamped to a finite band so
/// `10^|decimals|` never overflows to `inf` (which would poison the rounding to `NaN`); a `|decimals|`
/// beyond the f64 range rounds any finite magnitude to `0`, matching Excel (`FIXED(5,-400)="0"`).
...
/// Whether a LITERAL format code is outside the supported subset — the ONE source of truth consulted
/// by both the parse-time gate ([`validate_text_format`]) and eval ([`render_text_format`]), so what
/// parses and what renders can never drift. Unsupported = more than four `;`-sections, or a bracketed
/// group `[…]` that is not a recognised ELAPSED-time token (`[h]`/`[hh]`/`[m]`/`[mm]`/`[s]`/`[ss]`) —
/// i.e. a colour (`[Red]`) or a condition (`[>=100]`), which the v1 engine does not model. Everything
/// else (multi-section, currency, custom date/time, scientific, fractions) is supported.
...
/// Choose the section for a number by Excel's sign rule and render it. One section applies to all
/// numbers (a negative gets a leading `-`); two are `[non-negative; negative]`; three or four are
/// `[positive; negative; zero(; text)]`. From the second section on, the value is rendered in
/// MAGNITUDE (the section supplies its own sign/parentheses), never an auto minus. An EMPTY chosen
/// section renders as `""` (the `0;;;` idiom).
...
/// The index of the section Excel uses to format a TEXT value, or `None` when the code has no text
/// section. Excel's rule: a four-section code's 4th section is the text section (even without an `@`,
/// so `0;;;` hides text); otherwise the first section that carries an `@` text placeholder is the text
/// section wherever it sits (a lone `"@"`, or `"\"n/a\""`). `None` (a numeric-only code with fewer than
/// four sections) coerces numeric-looking text and, failing that, passes the text through UNCHANGED.
...
fn render_decimal(elems: &[Elem], value: f64) -> String {
    ...
    // Grouping: a `,` with a digit both before it and after it (in the integer part). Scaling: each
    // `,` after the last digit placeholder divides the value by 1000. `%` multiplies by 100.
    ...
    // Emit: literals in place; the whole integer block at the first integer placeholder; the point +
    // fraction at the point (only when a fraction is shown); subsequent digit tokens are absorbed.
    let mut out = String::new();
...
fn render_fraction(elems: &[Elem], value: f64) -> String {
    ...
    let den_digits = elems[slash + 1..]
        ...
        .count()
        .max(1);
    // A pathological placeholder run (`10^den_digits` overflows `u64` at 20+ digits) saturates the
    // cap; `best_fraction` is O(log max_den), so a huge cap can neither overflow nor hang (CORE2).
    let max_den = u32::try_from(den_digits)
        .ok()
        .and_then(|d| 10u64.checked_pow(d))
        .map_or(u64::MAX, |p| p - 1);
    // Numerator placeholders are the digit run immediately before the slash; any digit before THAT run
    // is the integer part.
...
/// The numerator/denominator with the least error approximating `x` in `[0, 1)` (or `[0, ∞)` for an
/// improper fraction) whose denominator is at most `max_den`; ties break to the smaller denominator.
///
/// Found via the continued-fraction convergents of `x` plus the best trailing semiconvergent: the
/// optimal bounded-denominator rational is always one of these, so this reproduces an exhaustive
/// `1..=max_den` scan (same least-error / smaller-denominator tie rule) in O(log max_den) — a 20+
/// digit denominator placeholder can therefore neither overflow nor hang (CORE2). All convergent
/// arithmetic is checked, so an improper fraction of a huge `x` bails to the best cap-fitting
/// candidate instead of overflowing.
fn best_fraction(x: f64, max_den: u64) -> (u64, u64) {
    ...
    for _ in 0..64 {
        ...
        match (hn, kn) {
            ...
            _ => {
                // This term overflows the denominator budget: the closest reachable rational on
                // this side is the largest semiconvergent a'·h1+h2 / a'·k1+k2 with denom <= max.
                if k1 > 0 {
                    let a_max = (max - k2) / k1;
                    if a_max >= 1 {
...
/// Round `|n|` half-away-from-zero to `decimals` places and split into `(is_negative,
/// integer_digits, fractional_digits)`. The sign is dropped when the rounded magnitude is zero.
///
/// Rounding is done in the scaled domain (`|n| * 10^decimals`) ONLY while that product stays within
/// f64's exact-integer range; beyond it `|n|`'s own ulp already exceeds `10^-decimals`, so rounding
/// to `decimals` places is the identity and we render `|n|` directly. The digits then come from Rust's
/// correctly-rounded float formatter — the exact f64→decimal expansion, matching the `formulas`
/// oracle (`FIXED(5,30)="5.0000…"`, `FIXED(1/3,30)="0.333333333333333314829616256247"`). This keeps
/// the output bounded for a large `decimals` (no `10^decimals` overflow to `inf`, no O(n^2) padding).
...
/// Parse-time gate for `TEXT`: refuse ONLY an UNSUPPORTED STRING LITERAL format code (see
/// [`format_unsupported`] — 5+ sections or a colour/condition bracket). A non-literal (computed)
/// format is ACCEPTED and deferred to `text_fn`, which returns `#VALUE!` iff the RESOLVED format is
/// unsupported (accept-under-uncertainty, ast-standards PART 6: a false-reject is the cardinal sin, a
/// deferred gap is a false-negative). Registered as `TEXT`'s `validate` row so the check stays registry
/// data, not a hand-fork in the parser.
