// Concern: UNIT-TEST pins for the math family built-ins (ABS ROUND MOD INT SQRT POWER ROUNDUP ROUNDDOWN CEILING FLOOR PRODUCT SUMPRODUCT) exercised through `FUNCS` dispatch — rounding tie/sign rules, the CEILING/FLOOR zero-significance asymmetry, and PRODUCT/SUMPRODUCT array semantics | Non-concern: the math impls (`func/math.rs`) and the shared test fixtures (the parent `tests` module owns `num`/`call`/`col_range`) | IO: in-memory `Grid` fixtures + literal `Expr`s -> asserted `Value`s
use super::*;
