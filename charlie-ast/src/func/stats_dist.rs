// Concern: the NORMAL-DISTRIBUTION worksheet functions and the z-score STANDARDIZE — the general normal density/CDF (NORM.DIST / legacy NORMDIST) and its inverse (NORM.INV / legacy NORMINV), the standard-normal density/CDF (NORM.S.DIST / legacy NORMSDIST) and inverse (NORM.S.INV / legacy NORMSINV) — each Excel-exact in arg order, the cumulative-flag switch (CDF vs PDF), and the `#NUM!` domain guards; the CDF uses a double-precision rational approximation (Φ accurate to ~1e-15) and the inverse a rational seed refined by one Halley step, so results match Excel within the ENG6 tolerance | Non-concern: the descriptive/ranking/regression statistics (func/stats*.rs), the registry table + dispatch (func/mod.rs), and the shared `one_num`/`opt_bool`/`finite_or_num` helpers | IO: (`EvalCtx`, the call's unevaluated arg `Expr`s) -> `Value`
use super::*;
...
/// `STANDARDIZE(x, mean, standard_dev)` — the z-score `(x − mean) / standard_dev`. A non-positive
/// `standard_dev` is `#NUM!` (Excel).
...
/// `NORM.DIST(x, mean, standard_dev, cumulative)` — the normal CDF (cumulative) or PDF.
pub(crate) fn norm_dist_fn(ctx: &mut EvalCtx, args: &[Expr]) -> Value {
    norm_dist(ctx, args)
}

/// `NORMDIST(x, mean, standard_dev, cumulative)` — the legacy spelling of [`norm_dist_fn`].
...
/// `NORMINV(probability, mean, standard_dev)` — the legacy spelling of [`norm_inv_fn`].
pub(crate) fn norminv_fn(ctx: &mut EvalCtx, args: &[Expr]) -> Value {
    norm_inv_dispatch(ctx, args)
}

/// `NORM.S.DIST(z, cumulative)` — the STANDARD normal CDF (cumulative TRUE) or PDF (FALSE).
...
/// `NORMSDIST(z)` — the legacy standard normal CDF (always cumulative; no flag argument).
...
/// `NORM.S.INV(probability)` — the inverse STANDARD normal CDF `Φ⁻¹(p)`. `p` outside `(0, 1)` is
/// `#NUM!` (Excel).
...
/// `NORMSINV(probability)` — the legacy spelling of [`norm_s_inv_fn`].
