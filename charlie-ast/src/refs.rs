...
/// are **meaning** (they change *how* the reference is written, and *which* sheet is named), so they
/// participate in equality — `A1` and `$A$1` differ, as do `A1` and `Sheet2!A1`. `col`/`row` are
/// stored zero-based so a reference renders back to A1 trivially. Because it carries an owned
/// [`SheetName`], `RefNode` is not `Copy` (unlike the resolved [`CellRef`]).
#[derive(Clone, Debug, PartialEq, Eq, Hash)]
pub struct RefNode {
    pub col: u32,
    pub row: u32,
...
/// `end_row_abs`) that distinguish a mixed range like `$E$2:E2` from `E2:E2`. They are **meaning**
/// (an absolute range is written differently from a relative one), so they participate in equality.
#[derive(Clone, Debug, PartialEq, Eq, Hash)]
